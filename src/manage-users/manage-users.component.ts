...
export class ManageUsersComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private serviceBack: BackService) {
  ...
  ngOnInit(): void {
    this.serviceBack.listAllUsers().subscribe( res => {
      this.users = res
    }, error =>{
      //error fetching data
    })
 ...
 */
  ...
  delete(user: User) {
    this.serviceBack.deleteUser(user.userId).subscribe( res => {
  ...
  edit(){
    ...
    if(this.userToEdit !== undefined){
      if(usrname != '' && firstName != '' && lastName != '') {
        this.serviceBack.updateUser(usrname,firstName,lastName, this.userToEdit.userId).subscribe(res => {
          this.ngOnInit()
          this.hiddenEdit = !this.hiddenEdit
        },error => {
          //error
