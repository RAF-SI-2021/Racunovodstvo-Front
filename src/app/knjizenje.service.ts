...
export class KnjizenjeService {
  ...
  getKnjizenja(): Observable<DnevnikKnjizenja[]>{
    ...
    return this.httpClient.get<DnevnikKnjizenja[]>('http://localhost:8080/api/knjizenje/all', {headers:headers});
  }
  pretrazi(brNaloga: string, od:Date, doo:Date, brDokFak: string, komentar: string, uzetOd: boolean, uzetDo: boolean): Observable<DnevnikKnjizenja[]>{
    ...
    return this.httpClient.get<DnevnikKnjizenja[]>('http://localhost:8080/api/knjizenje'+s,{headers:headers});
  }
