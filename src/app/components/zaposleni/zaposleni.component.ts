...
export class ZaposleniComponent implements OnInit {
  ...
  constructor(private formBuilder : FormBuilder, private service : ZaposleniService, private fakturaService: InvoiceService, private router: Router) {
  ...
  filter(){
    ...
    this.service.filterZaposleni(this.createQuery(ime, prezime, statusZaposlenog,radnaPozicija)).subscribe((response) =>{
      if(response.ok){
  ...
  createQuery(ime : string, prezime : string, statusZaposlenog : string, radnaPozicija : string){
    let query = '';
    if(ime == '' && prezime == '' && statusZaposlenog == '' && radnaPozicija == ''){
      return 'all';
    }
    if( ime !== ''){
      query += 'ime:' + ime + ',';
  ...
  dodajZaposlenog(){
    ...
    this.service.createZaposleni(ime, prezime, datumRodjenja, jmbg, pol, radnaPozicija, preduzece).subscribe((response) => {
      this.ngOnInit();
    });
  }
