...
export class BrutoBilansComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private brutoBilansService: BrutoBilansService, private kontnaGrupaService: KontnaGrupaService) {
    ...
    this.kontoGroupOd = this.formBuilder.group({
      ...
      potrazuje: ['', [Validators.required, Validators.pattern("^[0-9]*$")]]
    })
  ...
  readBrutoBilans() {
    ...
    for (let i = 0; i < this.kontneGrupe.length; i++) {
      if (this.kontneGrupe[i].brojKonta === this.brutoBilansForm.get('kontoOd')?.value.split(' ',1)[0]) {
        kontoOd = this.kontneGrupe[i]
      }
      if (this.kontneGrupe[i].brojKonta === this.brutoBilansForm.get('kontoDo')?.value.split(' ',1)[0]) {
        kontoDo = this.kontneGrupe[i]
      }
    }

    this.brutoBilansService.readAll(
      kontoOd,
      kontoDo,
  ...
  readKontos() {
    this.kontnaGrupaService.readAll().subscribe(readKontoResp => {
      ...
      this.sortKontos()
      this.kontneGrupeOptionDo = this.brutoBilansForm.get('kontoDo')?.valueChanges.pipe(
        map(value => this._filterDo(value))
      )
      this.kontneGrupeOptionOd = this.brutoBilansForm.get('kontoOd')?.valueChanges.pipe(
        map(value => this._filterOd(value))
      )
    })
  }
  ...
  private _filterDo(value: any): KontnaGrupa[] {
    ...
    return this.kontneGrupe.filter(option => ((option.nazivKonta.toLowerCase().includes(filterValue) || option.brojKonta.toLowerCase().includes(filterValue)) && this.moreThanOd(option.brojKonta)));
  ...
  private _filterOd(value: any): KontnaGrupa[] {
    ...
    return this.kontneGrupe.filter(option => ((option.nazivKonta.toLowerCase().includes(filterValue) || option.brojKonta.toLowerCase().includes(filterValue)) && this.lessThanDo(option.brojKonta)));
