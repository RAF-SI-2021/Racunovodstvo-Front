...
export class ObracunService {
	...
	getObracuni(): Observable<Obracun[]> {
		...
		return this.httpClient.get<Obracun[]>(
			environment.preduzeceServiceApi + `/api/obracun/all`,
			{
				headers: headers,
			}
		);
	...
	izvrsiTransakciju(id: number): Observable<any> {
		const headers = { Authorization: `Bearer ${this.jwt}` };
		return this.httpClient.get<Obracun[]>(
			environment.preduzeceServiceApi + `/api/obracun/obradi/` + id,
			{
				headers: headers,
			}
		);
	...
	getDanSifraTransakcijeId(): Observable<ObracunZaradeConfig> {
		...
		return this.httpClient.get<ObracunZaradeConfig>(
			environment.preduzeceServiceApi + `/api/obracun_zarade_config`,
			{
				headers: headers,
			}
		);
	...
	findByDate(datum: String): Observable<Obracun[]> {
		const headers = { Authorization: `Bearer ${this.jwt}` };
		return this.httpClient.get<Obracun[]>(
			environment.preduzeceServiceApi + `/api/obracun_zarade/` + datum,
			{
				headers: headers,
			}
		);
	...
	updateZaposleni(
		...
		return this.httpClient.put<any>(
			environment.preduzeceServiceApi + `/api/obracun_zaposleni`,
			{},
	...
	deleteZaposleni(obracunZaposleniId: number): Observable<any> {
		...
		return this.httpClient.delete<Obracun[]>(
			environment.preduzeceServiceApi +
				`/api/obracun_zaposleni/` +
				obracunZaposleniId,
			{
				headers: headers,
			}
