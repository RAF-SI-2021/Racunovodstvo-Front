...
import {KontnaGrupa, Konto} from "../../model/model";
...
export class KnjizenjeWidgetComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(!this.editing){
      let knt4 = new Konto(new KontnaGrupa("", ""), 0,0,false,false,false,false)
      let kontoGroup = this.formBuilder.group({
        konto: [knt4.kontnaGrupa.brojKonta, [Validators.required]],
        duguje: [knt4.duguje, [Validators.required, Validators.pattern("^[0-9]*$")]],
        potrazuje: [knt4.potrazuje, [Validators.required, Validators.pattern("^[0-9]*$")]]
      });
  ...
  private _filter(value: any): KontnaGrupa[] {
    ...
    return this.ktnGrps.filter(option => (option.nazivKonta.toLowerCase().includes(filterValue) || option.brojKonta.toLowerCase().includes(filterValue)));
  ...
  knjizenje() {
    for(let i = 0; i < this.kontos.length - 1; i++){
      if(this.kontos[i].kontnaGrupa !== this.kontoGroups[i].get('konto')?.value){
        alert('Konto broj ' + i + ' nije sacuvan. Kliknite na dugme Izmeni u tom redu kako biste sacuvali promene')
        return;
      }
      if(this.kontos[i].duguje !== this.kontoGroups[i].get('duguje')?.value){
        alert('Konto broj ' + i + ' nije sacuvan. Kliknite na dugme Izmeni u tom redu kako biste sacuvali promene')
        return;
      }
      if(this.kontos[i].potrazuje !== this.kontoGroups[i].get('potrazuje')?.value){
        alert('Konto broj ' + i + ' nije sacuvan. Kliknite na dugme Izmeni u tom redu kako biste sacuvali promene')
        return;
      }
    }
